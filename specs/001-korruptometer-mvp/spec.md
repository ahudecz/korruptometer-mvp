...
#### Case discovery via K-Monitor

K-Monitor (`adatbazis.k-monitor.hu`) is a Hungarian anti-corruption NGO that has hand-curated a press archive of corruption-tagged articles since 2007. We use their tag taxonomy as a **case-discovery layer** — a way to find clusters of articles about a single case across many primary outlets — but we never ingest K-Monitor's editorial content. K-Monitor is **not** an `OutletAdapter`; the articles we ultimately store come from the primary outlets the existing five adapters already cover. The reverified `robots.txt` on 2026-05-10 disallows `ClaudeBot` and `GPTBot` outright but leaves `/adatbazis/cimkek/<tag>` paths default-allowed for our identifying `Korruptometer-Bot` UA, with explicit `Allow` for the two-segment `/adatbazis/cimkek/<tag>/<outlet>` filter form and explicit `Disallow` for any deeper path.

- **FR-076**: A K-Monitor discovery worker MUST enumerate candidate tag slugs by parsing `https://adatbazis.k-monitor.hu/hirek` (the redirect target of `/adatbazis/cimkek/`); enumeration MUST be a separate step from per-tag fetching and MUST NOT be invoked more than once per 24 hours.
- **FR-077**: Tag slugs discovered by FR-076 MUST be surfaced to editors at `/admin/kmonitor-tags` for explicit approval; only editor-approved slugs progress to per-tag traversal. The system MUST NOT auto-promote tags into `Case` records, because K-Monitor's taxonomy mixes case-specific tags (e.g. `lounge-design-kft`) with conceptual / political tags (e.g. `fidesz`, `nav-vizsgalat`, `video`) that do not map one-to-one to Korruptométer cases.
- **FR-078**: For each approved tag, the discovery worker MUST traverse `/adatbazis/cimkek/<tag>?page=N` until exhaustion, going through `httpGet` so the shared 2 s/host gate, the identifying User-Agent, and the robots.txt cache apply identically to every other scrape. The worker MUST NEVER request paths with three or more segments after `/cimkek/` (explicit Disallow).
- **FR-079**: The discovery worker MUST extract per article only: the K-Monitor tag slug, the primary-outlet `sourceUrl` from `.card__metadata__item--source_url`, the Wayback `archiveUrl` from `.card__metadata__item--archive_url` (kept as a fallback for dead primary URLs), and the K-Monitor outlet-slug hint. The worker MUST NOT extract or persist K-Monitor's editorial excerpt (`.card__content`), K-Monitor's cached headline, or any K-Monitor-internal URL. Discovered `sourceUrl`s MUST be handed to the existing `OutletAdapter` matching the URL hostname; the resulting `NewsArticle` rows MUST be stored with `source` set to the matching primary outlet (never `k-monitor`).
- **FR-080**: When a discovered primary `sourceUrl` returns HTTP 404/410, the worker MAY fall back to fetching the corresponding `archiveUrl` from `web.archive.org`; archive fetches MUST go through `httpGet` against `web.archive.org` (subject to its own robots/UA rules) and the resulting article MUST still be stored with `source` set to the original primary outlet, with a `viaArchive = true` flag so editors can distinguish archived sources.
...
- **KMonitorTagCandidate** — A tag slug observed on `adatbazis.k-monitor.hu` together with editor-approval state. Carries the slug, the timestamp the slug was first seen, the timestamp the slug was last seen on the K-Monitor index (slugs that disappear MUST NOT be auto-deleted), an editor-approval state (`pending` / `approved` / `rejected`), an optional pointer to the `Case` the slug maps to once approved, and the count of articles discovered under the slug at the most recent traversal. K-Monitor itself is NOT modelled as a `Source` row; it is a discovery layer, not an outlet, and its slug never appears as the `source` of any `NewsArticle`.
...
- **SC-031**: Across one calendar week of K-Monitor discovery runs, **0** outbound HTTP requests are issued to paths with three or more segments after `/adatbazis/cimkek/` (the explicit-Disallow zone), and **0** rows persisted by the discovery worker carry K-Monitor's editorial excerpt, K-Monitor-cached headline, or `source = 'k-monitor'`.
...
- **K-Monitor partnership is preferred but not required.** A drafted Hungarian-language outreach email to `info@k-monitor.hu` requesting an official data partnership exists and remains unsent; sending it is an editorial decision that can unlock a curated full dataset that would obsolete the discovery worker. The discovery worker described in FR-076 through FR-080 is the robots-respecting fallback if no partnership is established. If a partnership is later established, FR-076 through FR-080 MAY be retired in favour of an authorised bulk import.
