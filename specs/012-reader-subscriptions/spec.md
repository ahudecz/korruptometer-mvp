...
| P3 | Create the email provider account and the sending domain. **At account creation, and before any send whatsoever, set the provider's send-log retention to 7 days or less** — the provider's logs carry recipient addresses, and a retention setting added after the first send does not delete what those sends already wrote. "Any send" includes the Gmail and Outlook inbox-placement test. If the plan in use does not permit configuring retention at all, stop and take it back to the maintainer as a choice between a paid tier and a different provider; after the first send that choice is already made | Owner: maintainer (manual, outside the repo) | Email delivery (FR-047…FR-055) |
