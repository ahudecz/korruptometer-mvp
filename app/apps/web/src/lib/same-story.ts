...
import { extractNameCandidates, decideSameStoryTier } from '@korr/scrapers';
...
export async function findSameStoryDuplicate(opts: {
  ...
  if (candidates.length === 0) return { duplicate: false };

  const db = getDb();
  const ilikeConds = candidates.map((c) => sql`unaccent(headline) ILIKE unaccent(${`%${c}%`})`);
  ...
  const rows = (await db.execute(sql`
    ...
    ORDER BY wsim DESC
    LIMIT 3
  `)) as unknown as Array<{ id: string; headline: string; excerpt: string; wsim: number }>;
